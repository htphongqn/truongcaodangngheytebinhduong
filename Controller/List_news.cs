...
namespace Controller
{
    public class List_news
    {
        ...
        public List<News_details_entity> Load_listnews(int _Catid)
        {
            try
            {
                ...
                var list = (from a in db.ESHOP_NEWS_CATs
                            ...
                            select new { b.NEWS_ID, b.NEWS_TITLE, b.NEWS_IMAGE3, b.NEWS_DESC, b.NEWS_SEO_URL, b.NEWS_URL, b.NEWS_ORDER, b.NEWS_ORDER_PERIOD, b.NEWS_PUBLISHDATE, c.CAT_SEO_URL }).OrderByDescending(n => n.NEWS_PUBLISHDATE).OrderByDescending(n => n.NEWS_ORDER).ToList();
        ...
        public List<Pro_details_entity> LoadTinXemNhieu(int type, int limit)
        {
            try
            {
                ...
                var list = (from a in db.ESHOP_NEWS_CATs
                            ...
                            select new { b.NEWS_ID, b.NEWS_TITLE,b.NEWS_COUNT, b.NEWS_IMAGE3, b.NEWS_PRICE1, b.NEWS_DESC, b.NEWS_SEO_URL, b.NEWS_URL, b.NEWS_ORDER_PERIOD, b.NEWS_PUBLISHDATE, c.CAT_SEO_URL }).OrderByDescending(n => n.NEWS_PUBLISHDATE).OrderByDescending(n => n.NEWS_COUNT).Take(limit).ToList();
                foreach (var i in list)
                {
